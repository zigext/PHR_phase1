...
class PostActivity extends React.Component {
    ...
    checkNursePin = async (pin, callback) => {
        ...
        // else
        //     callback(false, null)
        const path = `${SERVER_IP}${PIN_CODE}?pin_code=${pin}&appid=${this.props.userReducer.appId}` //pin_code=${pin}&appid=${this.props.userReducer.appId}  //appid=${this.props.UserReducer.appId}
        await fetch(path)
    ...
    renderBody = () => {
        switch (this.state.step) {
            ...
            case 2:
            //Post-HR & post-BP
                return <Step2Post useBLE={this.props.useBLE} peripheral={this.props.peripheral}  step={this.state.step} onStepChange={this.onStepChange} onDataChange={this.onDataChange} postHr={this.state.postHr} postBp={this.state.postBp} />
            case 3:
            //Post-test
